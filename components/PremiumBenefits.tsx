...
export default function PremiumBenefits(){
 ...
 return <div ref={root} className="haswolf-premium-root"><button type="button" onClick={()=>setOpen(v=>!v)} className={premium?"haswolf-premium-trigger is-premium":"haswolf-premium-trigger"}><span>🔥</span><b>Premium</b>{premium&&<em>{tier}</em>}</button>{open&&<aside className="haswolf-premium-panel"><header><div><small>HASWOLF ELITE</small><h2>Premium Üyelik</h2></div><button className="haswolf-panel-close" onClick={()=>setOpen(false)} aria-label="Premium panelini kapat">✕</button></header><div className="haswolf-premium-hero"><span>🔥</span><div><strong>{premium?`${tier.toUpperCase()} AKTİF`:"Premium Dünyasına Katıl"}</strong><small>{until?`${new Date(until).toLocaleDateString("tr-TR")} tarihine kadar`:"Hesabına özel avantajlar"}</small></div></div><ul><li>Altın ve alevli profil çerçevesi</li><li>Özel sohbet ve lonca rozeti</li><li>Öncelikli destek ve kampanyalar</li><li>Daha fazla favori ve fiyat alarmı</li><li>Premium etkinlik ve çekilişleri</li></ul><a href="/hesabim">Üyeliğimi görüntüle</a></aside>}</div>
}
