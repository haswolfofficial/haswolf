...
export default function CekilisPage() {
  ...
  async function join(raffle:Raffle){
    ...
    const{data:p}=await supabase.from("profiles").select("nickname").eq("id",s.session.user.id).maybeSingle();
  ...
  return <main className="haswolf-legal-shell px-4 py-8 sm:py-14"><div className="mx-auto max-w-7xl">
    <header className="haswolf-legal-card overflow-hidden p-7 sm:p-10">
      <p className="text-sm font-bold tracking-[.28em] text-[#d9aa4a]">HASWOLF ŞEFFAFLIK MERKEZİ</p>
    ...
    <section className="mt-6 grid gap-4 md:grid-cols-3">{[["1","Giriş yap","Google veya misafir hesabıyla güvenli oturum aç."],["2","Katıl","Aktif çekilişte tek dokunuşla kaydını oluştur."],["3","Şeffaf sonuç","Kazananlar arşivde sıra numarasıyla yayımlanır."]].map(([n,t,d])=><div key={n} className="haswolf-legal-card p-5"><span className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#d9aa4a] font-black text-black">{n}</span><h2 className="mt-4 text-lg font-bold">{t}</h2><p className="mt-2 text-sm leading-6 text-zinc-500">{d}</p></div>)}</section>
