...
export default function CekilisPage() {
  ...
  async function join(raffle:Raffle){
    ...
    if(!s.session?.user){location.href="/login?returnTo=%2Fcekilis";return}
  ...
  return <main className="raffle-page">
    <style jsx global>{`
      ...
      @media(max-width:900px){.raffle-hero-grid,.featured-strip,.raffle-grid{grid-template-columns:1fr}.steps,.info-grid{grid-template-columns:1fr}.raffle-metrics{margin-top:4px}.raffle-hero{padding:25px}.raffle-cover{height:150px}.admin-grid{grid-template-columns:1fr}.admin-head{align-items:flex-start;flex-direction:column}}
    ...
    <div className="raffle-wrap">
      <header className="raffle-shell raffle-hero">
        <div className="raffle-hero-grid" style={{display:"grid",gridTemplateColumns:"1.15fr .85fr",gap:28,alignItems:"center"}}>
          <div>
            ...
            <p className="raffle-sub">Özel ödüller, doğrulanmış katılımlar ve sunucu tarafında güvenli kazanan seçimi. Her çekiliş hesap bazında kaydedilir, mükerrer katılım engellenir ve sonuçlar kalıcı arşivde yayımlanır.</p>
            <div className="raffle-actions"><a href="/" className="rbtn">← Ana Sayfa</a><a href="/topluluk-kurallari" className="rbtn">Katılım Kuralları</a><a href="/login?returnTo=%2Fcekilis" className="rbtn rbtn-primary">Hesabınla Katıl</a></div>
          </div>
          <div className="raffle-metrics"><Metric label="Aktif Çekiliş" value={active.length}/><Metric label="Toplam Katılım" value={entries.length}/><Metric label="Tamamlanan" value={archived.length}/><Metric label="Kazanan" value={winners.length}/></div>
        </div>
        <div className="featured-strip"><div className="raffle-shell feature"><div className="feature-label">Öne çıkan ödül</div><div className="feature-value">✦ {topPrize}</div></div><div className="raffle-shell feature"><div className="feature-label">Sistem güveni</div><div className="trust-row"><span><b>✓</b> Tek hesap</span><span><b>✓</b> Sunucu seçimi</span><span><b>✓</b> Kalıcı kayıt</span></div></div></div>
      </header>

      <section className="steps"><Step n="01" title="Doğrulanmış giriş" text="Google hesabınla oturum aç. Misafir hesaplar çekilişlere katılamaz."/><Step n="02" title="Tek dokunuşla katılım" text="Aktif çekilişe bir kez katıl; sistem mükerrer kaydı otomatik engeller."/><Step n="03" title="Şeffaf sonuç" text="Kazananlar güvenli seçimle belirlenir ve sonuçlar arşivde kalıcı olarak yayımlanır."/></section>
      ...
      <section className="raffle-grid" style={{marginTop:16}}>{shown.map(r=>{
        ...
        return <article key={r.id} className="raffle-shell raffle-card">
          ...
          <div className="raffle-body">
            ...
            <div className="mini-grid"><Mini label="Ödül" value={r.prize}/><Mini label="Katılımcı" value={count(r.id)}/><Mini label="Kazanan" value={r.winner_count}/></div>
            ...
            {r.status==='active'&&!isSoon&&<button onClick={()=>join(r)} className="join-btn">✦ Çekilişe Katıl</button>}
            {isManager&&r.status==='active'&&<button onClick={()=>draw(r.id)} className="rbtn" style={{marginTop:10,width:"100%",color:'#ff8585',borderColor:'rgba(255,80,80,.3)'}}>Yönetici: Çekilişi Sonuçlandır</button>}
      ...
      {isManager&&<section className="raffle-shell admin"><div className="admin-head"><div><div className="raffle-kicker">YETKİLİ ALANI</div><h2>Yeni Çekiliş Oluştur</h2></div><p>Ana admin veya yetkilendirilmiş çekiliş yöneticileri kullanabilir.</p></div><form onSubmit={createRaffle} className="admin-grid"><input name="title" required placeholder="Çekiliş başlığı"/><input name="prize" required placeholder="Ödül"/><input name="banner_url" style={{gridColumn:'1/-1'}} placeholder="Kapak görseli URL (isteğe bağlı)"/><textarea name="description" required style={{gridColumn:'1/-1'}} placeholder="Detaylı açıklama" rows={4}/><label style={{fontSize:12,color:'#8e8a82'}}>Başlangıç<input name="starts_at" type="datetime-local" required style={{marginTop:7}}/></label><label style={{fontSize:12,color:'#8e8a82'}}>Bitiş<input name="ends_at" type="datetime-local" required style={{marginTop:7}}/></label><input name="winner_count" type="number" min="1" max="100" defaultValue="1"/><textarea name="rules" style={{gridColumn:'1/-1'}} placeholder="Katılım koşulları, yasaklar ve teslimat açıklaması" rows={6}/><button disabled={saving} className="join-btn" style={{gridColumn:'1/-1',marginTop:0}}>{saving?'Kaydediliyor...':'✦ Çekilişi Yayınla'}</button></form></section>}

      <section className="info-grid"><Info icon="◈" title="Şeffaflık" text="Her hesap aynı çekilişe yalnızca bir kez katılabilir. Sonuçlar çekiliş kimliğiyle arşivlenir."/><Info icon="⌾" title="Güvenlik" text="Kazanan seçimi tarayıcıda değil, sunucu tarafında güvenli rastgelelik kullanılarak yapılır."/><Info icon="✦" title="Teslimat" text="Kazananlarla yalnızca resmî HASWOLF kanallarından iletişim kurulur; ödül teslimi için ödeme istenmez."/></section>
    </div>
  </main>;
}
