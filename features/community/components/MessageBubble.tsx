...
function renderMentions(text: string, currentNickname = "") {
  ...
  return parts.map((part, index) => {
    ...
    const mine = Boolean(normalizedCurrent) && target.toLocaleLowerCase("tr-TR") === normalizedCurrent;
    return (
      <strong
        key={`${part}-${index}`}
...
export default function MessageBubble({ message, currentNickname = "", onReply, onReport }: MessageBubbleProps) {
  return (
    <article id={`chat-message-${message.id}`} className={`group flex max-w-2xl gap-3 ${message.isMine ? "ml-auto flex-row-reverse" : ""}`}>
      ...
      <div className={`min-w-0 ${message.isMine ? "text-right" : ""}`}>
        ...
        {message.replyTo && (
          <button type="button" onClick={() => document.getElementById(`chat-message-${message.replyTo?.id}`)?.scrollIntoView({ behavior: "smooth", block: "center" })} className={`mb-1 block max-w-full rounded-lg border-l-2 border-[#d9aa4a] bg-black/30 px-3 py-2 text-left text-xs ${message.isMine ? "ml-auto" : ""}`}>
        ...
        <div className={`break-words rounded-2xl px-4 py-3 text-sm leading-relaxed ${message.isMine ? "rounded-tr-sm bg-[#d9aa4a] text-black" : "rounded-tl-sm bg-zinc-900 text-zinc-200"}`}>
          {renderMentions(message.message, currentNickname)}
        </div>
        {(onReply || (!message.isMine && onReport)) && (
          <div className={`mt-1 flex gap-2 opacity-0 transition group-hover:opacity-100 ${message.isMine ? "justify-end" : ""}`}>
            {onReply && <button type="button" onClick={() => onReply(message)} className="text-[11px] font-semibold text-zinc-500 hover:text-[#e5b64e]">↩ Yanıtla</button>}
            {!message.isMine && onReport && <button type="button" onClick={() => onReport(message)} className="text-[11px] font-semibold text-zinc-600 hover:text-red-400">⚑ Raporla</button>}
          </div>
        )}
      </div>
    </article>
